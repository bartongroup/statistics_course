---
title: "Errors; statistical estimators"
output:
  html_document:
...
S <- sd(x)
xx <- seq(30, 150, 1)
gs <- data.frame(x=xx, y=dnorm(xx, mean=M, sd=S) * length(x) * bins)
...
g1 <- ggplot(bs, aes(x=y, y=x)) +
  ...
  geom_point(size=1.5) +
  labs(x = "Reads per gene", y="") +
  scale_y_continuous(expand=c(0,0), limits=c(0.5,1.5)) +
  scale_x_continuous(limits=lims, breaks=seq(0, 200, 20))
...
g2 <- ggplot(df, aes(x=x)) +
  ...
  geom_histogram(breaks=brks, fill=fill.colour, colour="black", size=0.3) +
  geom_line(data=gs, aes(x,y), colour="red") +
  scale_y_continuous(expand=c(0,0), limits=c(0, 9), breaks=seq(2, 10, 2)) +
...
ggsave(paste0(fig, "normal_gene.png"), plot=g, device="png", width=3.5, height=3.5, dpi=300)
```
...
d$type <- relevel(d$type, "Symmetric")
dm <- d %>% group_by(type) %>% summarise(mean=mean(value), median=median(value)) %>% mutate(i=as.integer(type))
...
ggsave(paste0(fig, "median.png"), plot=g, device="png", width=2.5, height=2.5, dpi=300)
```
...
mu <- 20 
sigma <- 5
sam.dist <- mclapply(1:npar, function(i) {
  # filling pre-defined matrix is faster that creating dynamic list
  d <- matrix(nrow=ns, ncol=2, dimnames=list(NULL, c("Mean", "Median")))
  for(k in 1:ns) {
    x <- rnorm(n, mu, sigma)
    d[k, ] <- c(mean(x), median(x))
  }
  as_tibble(d)
}, mc.cores=npar) %>%
  bind_rows()
...
```{r mean_median_sampling}
sd(sam.dist$Mean) 
sd(sam.dist$Median)
brks <- seq(8, 32, 0.2)
g <- ggplot() +
  theme_classic() +
  geom_outline(sam.dist$Mean, colour="royalblue3", breaks=brks) +
  geom_outline(sam.dist$Median, colour="seagreen4", breaks=brks) +
  scale_y_continuous(expand=c(0,0), limits=c(0,0.2)) +
  labs(x="Mass (g)", y="Density")

g
ggsave(paste0(fig, "mean_median_sampling.png"), plot=g, device="png", width=2.5, height=2.5, dpi=300)
