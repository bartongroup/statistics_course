...
knitr::opts_chunk$set(
  ...
  echo = FALSE,
  cache = TRUE,
  cache.path = "../cache/lecture4/",
  fig.path = "../cache/lecture4/"
)
fig <- "../fig/L4/"
...
```{r prepare_mice}
width <- 0.6
M <- tapply(mice1$Mass, mice1$Country, mean)
L <- tapply(mice1$Country, mice1$Country, length)
mice1$grand.mean <- mean(mice1$Mass)
mice1$Mean <- M[mice1$Country]
mice1$N <- L[mice1$Country]
mice1$group <- as.integer(mice1$Country)
mice1$n <- unlist(tapply(mice1$Country, mice1$Country, seq_along))
mice1$x <- as.integer(mice1$Country) + width*(mice1$n - mice1$N/2 - 1/2) / (mice1$N - 1)
...
plotVariance <- function(mice, means, within=TRUE) {
  ...
  g <- g +
    ...
    geom_point(aes(x=x, y=Mass, colour=Country)) +
...
g1 <- ggplot(tc) + 
  geom_line(aes(x=Time, y=Mass, colour=Treatment, group=Course), alpha=0.3) +
  geom_point(aes(x=Time, y=Mass, colour=Treatment, shape=Treatment), size=2) +
  scale_colour_manual(values=cbPalette) +
  labs(x="Time (week)", y="Body mass (g)") +
  theme(legend.position = "none") +
  ylim(0, 36)
g2 <- ggplot(tcm) +
  geom_line(aes(x=Time, y=M, colour=Treatment, group=Treatment), alpha=0.3) +
  geom_point(aes(x=Time, y=M, colour=Treatment, shape=Treatment), size=2) +
  geom_errorbar(aes(x=Time, ymin=M-SE, ymax=M+SE, colour=Treatment), width=0.2) +
  scale_colour_manual(values=cbPalette) +
  labs(x="Time (week)", y="Body mass (g)") +
  theme(legend.position = "none") +
  ylim(0, 36)
g3 <- ggplot(tc) + 
  geom_line(aes(x=Time, y=nMass, colour=Treatment, group=Course), alpha=0.3) +
  geom_point(aes(x=Time, y=nMass, colour=Treatment, shape=Treatment), size=2) +
  scale_colour_manual(values=cbPalette) +
  labs(x="Time (week)", y="Normalized body mass") +
  theme(legend.position = "none") 
g4 <- ggplot(tcm) +
  geom_line(aes(x=Time, y=nM, colour=Treatment, group=Treatment), alpha=0.3) +
  geom_point(aes(x=Time, y=nM, colour=Treatment, shape=Treatment), size=2) +
  geom_errorbar(aes(x=Time, ymin=nM-nSE, ymax=nM+nSE, colour=Treatment), width=0.2) +
  scale_colour_manual(values=cbPalette) +
  labs(x="Time (week)", y="Normalized body mass") +
  theme(legend.position = "none")
...
ggsave(paste0(fig, "time_course.png"), plot=g1, device="png", width=3.5, height=3.0, dpi=300)
ggsave(paste0(fig, "time_course_mean.png"), plot=g2, device="png", width=3.5, height=3.0, dpi=300)
ggsave(paste0(fig, "time_course_norm.png"), plot=g3, device="png", width=3.5, height=3.0, dpi=300)
ggsave(paste0(fig, "time_course_mean_norm.png"), plot=g4, device="png", width=3.5, height=3.0, dpi=300)
```
...
areaPlot <- function(d) {
  ...
  ggplot(d, aes(x=Treat, y=Area, colour=Treat)) +
    theme_classic() +
    theme(legend.position = "none") +
    #geom_boxplot(aes(fill=Treat), alpha=0.5, outlier.shape = NA) +
    scale_colour_manual(values=cbPalette[2:1]) +
    geom_jitter(width=0.15, height=0) +
    labs(x="", y="") +
