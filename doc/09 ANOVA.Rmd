...
g <- plotMiceBox(mice1, cex=2.6) 
g

ggsave(paste0(fig, "mice_1way.png"), plot=g, device="png", width=3.2, height=3.2, dpi=300)
```


```{r variance_within_between, fig.width=6}
set.seed(23143)
makeVar_ <- function(M, S=5, n=20) {
...
makeVar <- function(M, S=5, n=20) {
  M %>%
    setNames(countries) %>%
    map_df(rnorm, n=n, sd=S) %>%
    gather(key="Country", value="Mass") %>%
    mutate(x = as.integer(as.factor(Country)))
}


plotWB <- function(d, maxy=40) {
  aov <- aovResults(d)
  #d$x <- as.numeric(as.factor(d$Country))
...
g1 
g2 
ggsave(paste0(fig, "variance_within.png"), plot=g1, device="png", width=3.3, height=3, dpi=300)
ggsave(paste0(fig, "variance_between.png"), plot=g2, device="png", width=3.3, height=3, dpi=300)
```
...
g1 <- plotMiceBox(mice1, cex=2.6, limits=c(0,40))
g1

ggsave(paste0(fig, "mice_1way_effect.png"), plot=g1, device="png", width=3.2, height=3.2, dpi=300)

g2 <- plotMiceBox(dat, cex=2.6, limits=c(0,40)) 
g2

ggsave(paste0(fig, "mice_1way_no_effect.png"), plot=g2, device="png", width=3.2, height=3.2, dpi=300)
```



```{r mice_colour, fig.width=4, fig.height=3}
g1 <- plotMiceBox(subset(mice2, Colour=="White"), limits=c(0,42), cex=2.6)
g2 <- plotMiceBox(subset(mice2, Colour=="Black"), limits=c(0,42), cex=2.6)
g1
g2

ggsave(paste0(fig, "mice_white.png"), plot=g1, device="png", width=3.2, height=3.2, dpi=300)
ggsave(paste0(fig, "mice_black.png"), plot=g2, device="png", width=3.2, height=3.2, dpi=300)
```
