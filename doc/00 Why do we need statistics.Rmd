...
blood <- read.table("../data/normtemp.dat.txt", header=FALSE)
names(blood) <- c("temperature.f", "sex", "hrate")
blood$temperature.c <- (blood$temperature.f - 32) *5 / 9

M <- mean(blood$temperature.c)
S <- sd(blood$temperature.c)
x <- seq(35,39,0.01)
m <- data.frame(
  x = x,
...
g1 <- ggplot(blood, aes(x=temperature.c, y=1)) +
  theme_classic() +
  theme(
...
g2 <- ggplot() +
  ...
  geom_histogram(data=blood, aes(x=temperature.c, y=..density..), breaks=brks, fill=fill.colour) +
  geom_outline(blood$temperature.c, breaks=brks) +
...
ptpn <- read_tsv("../data/tumour_ptpn.txt") %>%
  mutate(day = as.numeric(gsub("day", "", X1))) %>%
  select(-1) %>%
  setNames(c(paste0("KO_", 1:12), paste0("WT_", 1:12), "day")) %>%
...
```{r genes}
genes <- c("ensmusg00000051951", "ensmusg00000009281","ensmusg00000013415")
for(gene in genes) {
...
genes <- c("ensmusg00000023011", "ensmusg00000031671", "ensmusg00000043614")
for(gene in genes) {
