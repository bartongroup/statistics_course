...
ptpn <- read_tsv("../data/tumour_ptpn.txt") %>%
  ...
  select(-1)
names(ptpn) <- c(paste0("KO_", 1:12), paste0("WT_", 1:12), "day")
...
g1 <- ggplot(ptpn, aes(x=day, y=volume, group=sample, colour=condition)) +
    ...
    theme(legend.position = "none", plot.title=element_text(size=t.size))
...
g3 <- ggplot(lp, aes(x=lday, y=lvolume, colour=condition)) +
  theme_classic() +
  theme(
