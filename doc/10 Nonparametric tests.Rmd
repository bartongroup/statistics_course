...
```{r rank_plot_4mice, fig.width=3, fig.height=3}
ls2 <- subset(lifespan4, Country %in% c("English", "Scottish"))
ls2$Rank <- rank(ls2$Lifespan, ties.method="random")
g2.1 <- plotMiceBox(ls2, cex=7, size=2, what="Lifespan", ylab="Lifespan (days)", with.shape=TRUE)
g2.2 <- plotMiceBox(ls2, size=2, what="Rank", ylab="Rank", with.shape=TRUE) +
...
ggsave(paste0(fig, "value_rank.png"), plot=g0, device="png", width=3.3, height=3.5, dpi=300)
...
generateHdist <- function(n=c(12, 9, 8, 5), nsim=1000) {
  P <- sapply(1:nsim, function(i) {
...
kolmogorovDist <- function(x, n1, n2) {
  sn <- sqrt(n1 * n2 / (n1 + n2))
  #x <- seq(0, 1, delta)
  delta <- x[2] - x[1]
...
g1 <- plotMiceBox(subset(ls4, Country %in% countries), what="Lifespan", ylab="Lifespan (days)", cex=5, size=1.8)
...
g1 <- plotMiceBox(droplevels(subset(ls4, Country %in% countries)), cex=5, size=1.8, what="Lifespan", ylab="Lifespan (days)",)
g2 <- plotCumsum(ls4, vars=countries)
g <- plot_grid(g1, g2, ncol=2, rel_widths = c(1, 1.4))
g
...
g1 <- plotMiceBox(ls.gen, what="Lifespan", ylab="Lifespan (days)")
...
plotDdist <- function(D, nx, ny, d.cut=0.42, ymax=5) {
  ...
  #gs <- dks(x, ne)
  gs <- kolmogorovDist(seq(0, 1, 0.001), nx, ny)
 
  g2 <- g1 +
    geom_line(data=gs, aes(x, f), colour="red")
  g2

  ff <- kolmogorovDist(seq(d.cut, 1, 0.001), nx, ny)
  gc <- data.frame(
    x = c(d.cut, ff$x, 1),
    y = c(0, ff$f, 0)
  )
