...
plotMiceBox <- function(mice, what="Lifespan", ylab="Lifespan (day)", seed=111, limits=NULL, width=0.15, cex=4, size=2, withbox=TRUE, palette=british.palette) {
  set.seed(seed)
  g <- ggplot(mice, aes_string(x="Country", y=what)) +
    theme_classic() +
    theme(legend.position = "none")
  if(withbox) g <- g + geom_boxplot(aes(fill=Country), alpha=0.4, outlier.shape=NA, colour="grey70")
  g <- g +
    scale_fill_manual(values=palette) +
    #geom_jitter(width=width, height=0) +
    geom_beeswarm(cex=cex, size=size, aes(fill=Country), shape=21, priority="density") +
    labs(x="", y=ylab)
...
g2 <- plotMiceBox(lifespan4, what="Rank", ylab="Rank", width=0, cex=3.1)
...
g2.1 <- plotMiceBox(ls2, cex=5, size=1.5)
g2.2 <- plotMiceBox(ls2, size=1.5, what="Rank", ylab="Rank", width=0)
