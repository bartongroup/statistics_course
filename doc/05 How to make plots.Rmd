...
d <- sapply(1:nrow(dat), function(i) {
  prop.test(dat$s[i], dat$n[i])$conf.int
}) %>%
  t %>%
  as_tibble %>%
  setNames(c("lo", "up")) %>%
...
ggsave(paste0(fig, "colour_blind_1.png"), plot=g1, device="png", width=4, height=3, dpi=300)
ggsave(paste0(fig, "colour_blind_2.png"), plot=g2, device="png", width=4, height=3, dpi=300)
```