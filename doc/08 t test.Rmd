...
plotOneSample <- function(x, m, with.point=FALSE, limits=NULL, point.colour="blue", error.type="SE") {
  df <- data.frame(x = x)
...
g1 <- plotFun(dt, df=dof, x.grid=seq(-5,5,0.01), name="t")
g2 <- plotFun(dt, df=dof, x.grid=seq(-5,5,0.01), cut.up=t.obs, name="t")
g3 <- plotFun(dt, df=dof, x.grid=seq(-5,5,0.01), cut.lo=-t.obs, cut.up=t.obs, name="t")

#lst <- tCut(t.obs, dof)
ggsave(paste0(fig, "t_dist.png"), plot=g1, device="png", width=2.5, height=2, dpi=300)
ggsave(paste0(fig, "t_dist_cut.png"), plot=g2, device="png", width=2.5, height=2, dpi=300)
ggsave(paste0(fig, "t_dist_cut2.png"), plot=g3, device="png", width=2.5, height=2, dpi=300)
...
g <- plotOneSample(x5, 20, limits=c(min(x5)-2, max(x5)+2), with.point=TRUE, error.type="CI")
g

ggsave(paste0(fig, "CI_vs_test_5.png"), plot=g, device="png", width=3.5, height=3.5, dpi=300)
```
...
g1 <- plotOneDist(md$t.eq, "t", "", c(-4, 4), NULL)
g2 <- plotOneDist(md$t.eq, "t", "", c(-4, 4), dt, N1 + N2 - 2)
 
ggsave(paste0(fig, "two_sample_tdist.png"), plot=g1, device="png", width=3.5, height=3, dpi=300)
ggsave(paste0(fig, "two_sample_tdist_t.png"), plot=g2, device="png", width=3.5, height=3, dpi=300)
```
...
ggsave(paste0(fig, "mouse_eng_sco.png"), plot=g, device="png", width=2.5, height=3.5, dpi=300) 
```


```{r ttest_var, fig.width=3, fig.height=3}
#lst.eq <- tCut(t.obs=mice.prop$t.eq, mice.prop$nu.eq)
#lst.ne <- tCut(t.obs=mice.prop$t.ne, mice.prop$nu.ne)
#lst.eq$t.one
#lst.ne$t.one
 
g1 <- plotFun(dt, df=mice.prop$nu.eq, x.grid=seq(-5,5,0.01), cut.up = mice.prop$t.eq, name="t")
g2 <- plotFun(dt, df=mice.prop$nu.ne, x.grid=seq(-5,5,0.01), cut.up = mice.prop$t.ne, name="t")

g1
g2

ggsave(paste0(fig, "t_eq.png"), plot=g1, device="png", width=2.5, height=2, dpi=300)
ggsave(paste0(fig, "t_ne.png"), plot=g2, device="png", width=2.5, height=2, dpi=300)
```
...
plotCI <- function(x1, x2) {
  ...
  g <- ggplot() +
    ...
    geom_beeswarm(data=d, aes(x=x, y=y, fill=x), shape=21, cex=2.6) +
...
ggsave(paste0(fig, "ttest2_p_ci.png"), plot=g, device="png", width=6, height=3, dpi=300)
```
...
plotMice <- function(dat) {
  ...
  g <- ggplot(d, aes(x=Country, y=Mass)) +
    theme_classic() +
    theme(legend.position = "none") +
    geom_boxplot(aes(fill=Country), alpha=0.5, colour="grey50") +
    scale_fill_manual(values=british.palette) +
    #geom_jitter(width=0.15, height=0) +
    geom_beeswarm(cex=2.5, size=1, priority="density") +
...
g1 <- plotMiceBox(mkDat(dat1), cex=2.5, size=1, limits=lms)
g2 <- plotMiceBox(mkDat(dat2), cex=2.5, size=1, limits=lms)
...
g1 <- g + geom_point()
g2 <- g + geom_point() + geom_segment(data=x, aes(x="Before", xend="After", y=Before, yend=After, colour=sign)) + geom_point() 

ggsave(paste0(fig, "ttest_unpaired.png"), plot=g1, device="png", width=2.5, height=3.5, dpi=300)
ggsave(paste0(fig, "ttest_paired.png"), plot=g2, device="png", width=2.5, height=3.5, dpi=300)
```
