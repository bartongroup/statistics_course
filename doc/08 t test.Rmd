...
while(!done) {
  ...
  done <- abs(p - 0.05) < 1e-5 & abs(sd(x1) - sd(x2)) < 0.2
}
m1 <- mean(x1)
m2 <- mean(x2)
...
g <- ggplot() +
  ...
  geom_beeswarm(data=d, aes(x=x, y=y, fill=x), shape=21, cex=2) +
...
ggsave(paste0(fig, "ttest2_p_ci.png"), plot=g, device="png", width=4, height=3, dpi=300)
```
