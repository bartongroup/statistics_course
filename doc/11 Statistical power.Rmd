...
plotMiceBox <- function(mice, seed=111, limits=NULL, cex=4, size=2, with.means=FALSE, with.boxes=TRUE) {
  set.seed(seed)
  g <- ggplot() +
    theme_classic() +
    theme(legend.position = "none") +
    scale_fill_manual(values=british.palette) +
    geom_beeswarm(data=mice, aes(x=Country, y=Mass, fill=Country), shape=21, cex=cex, size=size, priority = "density") +
    labs(x="", y="Body mass (g)")
  if(!is.null(limits)) g <- g + ylim(limits)
  
  if(with.means) {
    m <- mice %>% group_by(Country) %>% summarise(M = mean(Mass))
  ...
  if(with.boxes) {
    g <- g + geom_boxplot(data=mice, aes(x=Country, y=Mass, fill=Country), alpha=0.4, colour="grey60", outlier.shape = NA)
  }
...
g1 <- plotMiceBox(d1, size=1.5, cex=2, with.boxes=FALSE, with.means=TRUE) + ylim(y1, y2)
g2 <- plotMiceBox(d2, size=1.5, cex=2, with.boxes=FALSE, with.means=TRUE) + ylim(y1, y2)

g1
g2

ggsave(paste0(fig, "anova_f1_1.png"), plot=g1, device="png", width=3.5, height=3.5, dpi=300)
ggsave(paste0(fig, "anova_f1_2.png"), plot=g2, device="png", width=3.5, height=3.5, dpi=300)
