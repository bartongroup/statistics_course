...
ggsave(paste0(fig, "sampling_distributions.png"), plot=g, device="png", width=5, height=5, dpi=300)
```
...
x <- 2:30
g1 <- cPlot(x=x, y=qt(0.975, x-1), "t*", qnorm(0.975), ylim=c(0,13), breaks=seq(2,14,2))
g2 <- cPlot(x=x, y=1 - 2*pt(-1, x-1), "Confidence of SE", 1 - 2*pnorm(-1))
g <- plot_grid(g1, g2, ncol=1, align="v")
g

ggsave(paste0(fig, "se_and_ci.png"), plot=g, device="png", width=4, height=5, dpi=300)
