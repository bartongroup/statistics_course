...
plotOneThing <- function(dat, what, with.error=FALSE) {
  g <- ggplot(dat) +
    theme_classic() +
    theme(legend.position = "none") +
    geom_point(aes_string(x="Condition", y=what)) +
    labs(x="", y=what)
  if(with.error) {
...
g1 <- plotOneThing(dat, "Value", with.error = TRUE)
...
g <- ggplot(Udist) +
  theme_classic() +
  geom_line(aes(dm, ux), colour="red", size=0.3) +
  geom_line(aes(dm, uy), colour="darkgreen", size=0.3) +
  geom_line(aes(dm, u), colour="blue", size=1) +
...
g <- ggplot(wdat, aes(dm, W)) +
  ...
  labs(x="Difference between the means", "W")
g
...
plotMiceBox <- function(mice, what="Lifespan", ylab="Lifespan (day)", seed=111, limits=NULL) {
  set.seed(seed)
...
g2 <- plotMiceBox(lifespan4, what="Rank", ylab="Rank")
...
g2.2 <- plotMiceBox(ls2, what="Rank", ylab="Rank")
