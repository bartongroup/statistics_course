...
plot_mix_dist <- function(d, breaks, alpha = 1) {
  d0 <- d |> filter(hyp)
  d1 <- d |> filter(!hyp)
  ggplot() +
    theme_dist +
    ...
    geom_histogram(aes(x = value), data = d0, fill = ef_palette[1], colour = NA, breaks = breaks, alpha = alpha) +
    geom_outline(d0$value, breaks, y.value = "count", colour = "grey40", alpha = alpha)+
    geom_histogram(aes(x = value), data = d1, fill = ef_palette[2], colour = NA, breaks = breaks, alpha = alpha) +
...
plot_mix_p <- function(p, x.lim = c(0, 1), bins = 0.01, alpha = 0.8, ymax = 5, x.breaks = seq(0,1,0.2)) {
  ...
  ggplot() +
    ...
    scale_colour_manual(values = ef_palette[2:1]) +
    scale_x_continuous(breaks = x.breaks, expand = c(0,0)) +
    coord_cartesian(xlim = x.lim) +
    geom_histogram(data = p, aes(x = p, y = after_stat(count)/(sum(after_stat(count)) * bins), fill = hyp, colour = hyp),
                   breaks = brks, position = "stack") +
...
plot_effect_sizes <- function(eff) {
  ...
  g1 <- ggplot(eff) +
    ...
    geom_histogram(aes(x = M, fill = sig, colour = sig), breaks = brks) +
    scale_x_continuous(expand = c(0,0)) +
    scale_fill_manual(values = ef_palette) +
    scale_colour_manual(values = ef_palette) +
    geom_outline(eff$M, brks, y.value = "count", colour="grey30", linewidth = 0.3) +
...
plot_strain_dist <- function(d) {
  ...
  g1 <- ggplot(m) +
    ...
    scale_y_continuous(expand = c(0,0), limits = c(0, max(m$M + m$SE)*1.3)) +
  ...
  g2 <- ggplot() +
    ...
    geom_quasirandom(data = d, aes(x = strain, y = distance), size = 0.1, width = 0.2, method = "quasirandom",
                     bandwidth = 0.1, varwidth = TRUE) +
    labs(x = NULL, y = ~ "Distance ("*mu*m*")") +
    geom_boxplot(data = m, aes(x = strain, middle = M, ymin = M, lower = M, upper = M, ymax = M), stat = "identity",
                 width = 0.5, lwd = 0.6, fatten = 0, colour = "darkgoldenrod3")
