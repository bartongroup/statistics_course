...
plot_mini_normal <- function(n = 10000) {
  ...
  ggplot(aes(x = d, y = after_stat(density))) +
    theme_classic() +
    theme(
...
plot_median <- function(seed = 112, n = 10) {
  ...
  ggplot() +
    ...
    geom_segment(data = dm, aes(x = i-shift, xend = i+shift, y = mean, yend = mean), linewidth = 0.9, colour = "royalblue3", lineend = "round") +
    geom_segment(data = dm, aes(x = i-shift, xend = i+shift, y = median, yend = median), linewidth = 0.9, colour = "seagreen4", lineend = "round") +
...
plot_sampling_mean <- function(sam, lims = c(0, 40), nsub = 8, seed = 123, cex = 2, size = 1, bins = 100, with.errors = FALSE) {
  ...
  g2 <- ggplot(data.frame(d = sam_mean), aes(x = d, y = after_stat(density))) +
    theme_dist +
